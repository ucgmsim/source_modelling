...
cdef void read_slipt_values(FILE* srf_file, int start_column, int slip_count, sparse_matrix* slip_matrix):
    ...
    if slip_count > 0:
        ...
        if slip_matrix.col_ptr == NULL:
            slip_matrix.len = max(slip_matrix.entries, 64)
            slip_matrix.col_ptr = <ITYPE_t*> malloc(slip_matrix.len * sizeof(ITYPE_t));
            slip_matrix.data = <DTYPE_t*>malloc(slip_matrix.len * sizeof(DTYPE_t));
        elif slip_matrix.entries > slip_matrix.len:
            while slip_matrix.len < slip_matrix.entries:
                  slip_matrix.len = slip_matrix.len + (slip_matrix.len >> 1) # slip_matrix length = 1.5 x old length
            slip_matrix.data = <DTYPE_t*>realloc(slip_matrix.data , slip_matrix.len * sizeof(DTYPE_t));
            slip_matrix.col_ptr = <ITYPE_t*>realloc(slip_matrix.col_ptr, slip_matrix.len  * sizeof(ITYPE_t));
        if slip_matrix.col_ptr == NULL or slip_matrix.data == NULL:
            raise MemoryError("Failed to allocate memory for column and data pointers.")

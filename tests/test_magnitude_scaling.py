...
def relation_with_magnitude(
    ...
    return sampler()  # ty: ignore[missing-argument] - @st.composite strips the `draw` parameter; sampler() is valid with no args
...
def test_inversion(
    ...
    assert area_to_mag(mag_to_area(magnitude)) == pytest.approx(magnitude)  # ty: ignore[missing-argument] - rake is handled by functools.partial above for relations that require it
...
def test_monotonicity_mag_to_area(
    ...
    if scaling_relation == magnitude_scaling.ScalingRelation.LEONARD2014:
        ...
        mag_to_area = MAGNITUDE_TO_AREA[scaling_relation]
    assert mag_to_area(magnitude + 0.1) > mag_to_area(magnitude)  # ty: ignore[missing-argument] - rake is handled by functools.partial above for relations that require it
